...
function Cart({setIsOpen,cartData,setCartData}) {
  ...
  return (
   createPortal((<div className="cart_modal">
     <section className="cart_container">
      ...
      {Object.keys(cartData).length > 0 ? (<div className="checkout_section">
        ...
        <button className="checkout_button cart_btn_primary" onClick={()=>setIsOpen(false)}>
          <span className="checout_button_text">Proceed to Checkout</span>
          <span className="item_checkout_price">{`Rs.${totalPrice} >`}</span>
        </button>
      </div>) : ( <button className="start_shopping cart_btn_primary" onClick={()=>setIsOpen(false)}>
            Start Shopping
        </button>)}
    </section>
